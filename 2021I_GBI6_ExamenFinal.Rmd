...
```{r wd}
getwd()
setwd("C:/Users/Usuario/Desktop/bio/GBI6_ExamenFinal")
```

### [0.5 PUNTS] 1.2. Cargue la data "mRNA_expr.csv"
```{r data}
...
```{r funcion_long}
library("tidyr")
long_df <- df %>% pivot_longer(!c(bcr_patient_barcode, dataset), names_to = "Gen", values_to = "expresion_level")
...
library("sjmisc")
library("dplyr")
descript_table <- df_long %>% group_by(dataset,Gen)
TDesc <- descr(descript_table, out="browser",file="results/mRNA_expr_summary.doc")
library(readxl)
mi_df <- data.frame(TDesc)
...
tcga_boxplots <- function(data){
  datas <- df_long %>% filter (dataset == data)
  data_final <- datas %>% ggplot(aes(x=Gen,y=expresion_level, color = Gen)) +  geom_boxplot() + geom_jitter ()}
...
```{r list_boxplots}
graphic <- unique(df_long$dataset)
cancertype_boxplots <- lapply(graphic, tcga_boxplots)
o <- length(cancertype_boxplots)
print (paste("numero de elementos: ", o))
...
library(ggplot2)
library(ggpubr)
grafico_3 <- cancertype_boxplots[[3]]
ggsave(filename = "results/boxplot3.png", plot = grafico_3,  units="in", dpi = 300)
...
library(dplyr)
library(ggplot2)
reg_gen_expression <- function(grafico_3){
  ejex=grafico_3[1]
  ejey=grafico_3[2]
  graphic_point1 <- df_long %>% filter (Gen == ejex)
  graphic_point2  <- df_long %>% filter (Gen == ejey)
  graphic_point <- cbind(graphic_point1, graphic_point2)
  final1 <-  tibble("dataset"=graphic_point[,1],"gen1"=graphic_point[,3],"gen2"=graphic_point[,6])
  final <- final1 %>% ggplot(aes(x = gen1, y = gen2, color=dataset)) + geom_point() + geom_smooth(method=lm, se=FALSE)
  final}                                                 
...
```{r list_regplots}
combination <- unique(df_long$Gen)
combination1 <- combn(combination,combination2=2,simplify = FALSE)
reg_genplots <- lapply(combination1,reg_gen_expression)
print (paste("numero de la lista es: ", length(reg_genplots)))
print ("El 11vo elemnto es:") 
reg_genplots[[11]] 
```
...
```{tex}
#Tuve problemas para visualizar el gráfico por  lo que no puedo hacer la interpretación adjunto en resultados el problema de r , hable con el profe acerca de este asunto                            
```
